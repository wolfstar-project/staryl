...
export function fetchLimits(guildId: string | bigint) {
  return Json<RingLimits>(
    safeTimedFetch(`${InternalRingUrl}/${guildId}`, 1000, {
      headers: InternalRingHeaders,
    }),
  );
}

export const DefaultLimits = {
  maximumYouTubeSubscriptions: 3,
  maximumTwitchSubscriptions: 5,
} as const satisfies RingLimits;
